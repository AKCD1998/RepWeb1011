...
// ===== DEBUG BOOT (ช่วยเช็คว่า Railway รันไฟล์นี้จริง) =====
console.log("BOOT: API server.js loaded");
...
// ===== API ROUTES (ต้องอยู่ก่อน static/fallback เสมอ) =====
...
app.get("/api/products", async (req, res) => {
  ...
  res.json({
    ...
    companyCode: product.product_code ?? product.company_code ?? null,
    name: product.brand_name ?? product.product_name ?? product.name ?? null,
    price: product.price_baht ?? product.price ?? null,
    qtyPerUnit: product.qty_per_unit ?? product.qty_perunit ?? null,
    unit: product.unit ?? null,
  });
});

// 1.5) list products for db edit table
app.get("/api/products/list", async (req, res) => {
  console.log("HIT /api/products/list");
  const r = await pool.query(
...
// 1.6) snapshot for POS prefetch
app.get("/api/products/snapshot", async (req, res) => {
  const r = await pool.query(
    `SELECT
       barcode,
       product_code AS "companyCode",
       brand_name AS "name",
       price_baht AS "price",
       NULL::numeric AS "qtyPerUnit",
       NULL::text AS "unit"
     FROM products
...
// optional version/etag-like endpoint
app.get("/api/products/version", async (req, res) => {
  const r = await pool.query(
    "SELECT COUNT(*)::int AS count, COALESCE(MAX(id), 0) AS max_id FROM products"
  );
  const { count, max_id } = r.rows[0] || { count: 0, max_id: 0 };
  res.json({ version: `${count}-${max_id}` });
});
...
function normalizeOptionalId(value, label) {
  ...
  if (!Number.isInteger(id)) throw new Error(`${label} must be integer`);
  return id;
}
...
app.post("/api/products", async (req, res) => {
  ...
  if (!productCode || !brandName || priceBaht === undefined || priceBaht === null) {
    return res.status(400).json({
      error: "productCode, brandName, priceBaht are required",
    });
  }
  ...
  try {
    ...
    const r = await client.query(
      ...
      [
        ...
        resolvedDosageFormId ?? null,
        resolvedManufacturerId ?? null,
        Number(priceBaht),
        resolvedPurchaseLimitId ?? null,
    ...
    if (packSizeIdsFinal.length || Array.isArray(packSizes)) {
      await replaceProductLinks(client, "product_pack_sizes", "pack_size_id", productId, packSizeIdsFinal);
    }
    if (routeIdsFinal.length || Array.isArray(routes)) {
      await replaceProductLinks(client, "product_routes", "route_id", productId, routeIdsFinal);
    }
    if (reportIdsFinal.length || Array.isArray(reports)) {
      await replaceProductLinks(client, "product_reports", "report_id", productId, reportIdsFinal);
    }
...
app.post("/api/products/:id/pack-sizes", async (req, res) => {
  ...
  if (packSizeIds.length && !ids.length) return res.status(400).json({ error: "packSizeIds must be integers" });

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("DELETE FROM product_pack_sizes WHERE product_id = $1", [productId]);
...
app.post("/api/products/:id/routes", async (req, res) => {
  ...
  if (routeIds.length && !ids.length) return res.status(400).json({ error: "routeIds must be integers" });

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("DELETE FROM product_routes WHERE product_id = $1", [productId]);
...
app.post("/api/products/:id/reports", async (req, res) => {
  ...
  if (reportIds.length && !ids.length) return res.status(400).json({ error: "reportIds must be integers" });

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("DELETE FROM product_reports WHERE product_id = $1", [productId]);
...
// ===== IMPORTANT: if /api route not found, return JSON (prevents HTML fallback confusion) =====
app.use("/api", (req, res) => {
  res.status(404).json({ error: "API route not found" });
});

// ===== OPTIONAL: Serve static (ONLY if you want Railway to serve your frontend too) =====
