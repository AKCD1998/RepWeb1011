...
- `REACTjs-Project/server/db/pool.js`
  - Reads `DATABASE_URL`.
  - Creates one `pg.Pool` when `DATABASE_URL` is present.
...
- PostgreSQL.
- Runtime connection is controlled by one `DATABASE_URL`.
- Local simulation standard database is `localhost:55433/rx1011_local`.
- Render/live database is expected to be supplied through Render's `DATABASE_URL`.
...
| `DATABASE_URL` | `server/db/pool.js` | PostgreSQL connection string; required for DB-backed routes |
| `PORT` | `server/index.js` | API port; defaults to `5050` |
| `CORS_ORIGIN` | `server/index.js` | Comma-separated allowed origins; defaults to `http://localhost:5173` |
| `PATIENTS_CSV_PATH` | `server/index.js` | Optional CSV fallback path for `GET /api/patients` when DB is unavailable |
| `JWT_SECRET` | `authController.js`, `authMiddleware.js` | JWT signing/verification secret |
...
| Variable | Used by | Purpose/default |
|---|---|---|
| `DATABASE_URL` | Migration, audit, export, local scripts | Connection string for the active profile |
...
- `NODE_VERSION=20`
- `DATABASE_URL` with `sync: false`
- `JWT_SECRET` generated by Render
- `CORS_ORIGIN` with `sync: false`
...
- The primary backend has a CSV fallback for patients when `DATABASE_URL` is missing. That may be useful locally but is an unclear behavior for a shared production service.
...
- Replace Render-specific assumptions with shared-service configuration names, but preserve the single `DATABASE_URL` contract unless the shared platform requires another pattern.
- Revisit `CORS_ORIGIN` after consolidation. Same-origin deployment may remove the need for broad CORS configuration.
